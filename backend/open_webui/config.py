...
# Only sync static files from the frontend build when that build actually
# exists. In dev mode the SvelteKit dev server runs separately and never
# produces FRONTEND_BUILD_DIR, so without this guard the cleanup loop below
# would wipe the committed defaults in STATIC_DIR (favicon.ico, logo.png, …)
# without anything to copy back.
